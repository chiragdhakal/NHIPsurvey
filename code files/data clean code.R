...
section0 <- section0 %>%
  mutate(
    palika = case_when(
      uid == "5d7d33e1-7d7b-4465-bb24-4bb81df62331" ~ 51106, 
      TRUE ~ palika
    )
  )
...
section2b <- section2b %>% 
  ...
  mutate(
    ...
    v233j %in% c("ALSO BECAUSE PALIKA DID IT FOR THEM FOR FREE", "BABU RED CARD WALA DISABLED CHILD", 
                 ...
                 "कम्पनी आफैंले गरिदिएको") ~ "11 Free ma hune bhayekole",
...
wealth_urban <- wealth_urban %>%
  mutate(
    wealth_score = pca_urban$x[, 1],
    wealth_quintile = ntile(wealth_score, 5),
    wealth_quintile = factor(
      wealth_quintile,
      levels = 1:5,
      labels = c("Urban Poorest", "Urban Poorer", "Urban Middle", "Urban Richer", "Urban Richest")
    )
  )
