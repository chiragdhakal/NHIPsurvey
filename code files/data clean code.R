...
section6c2 <- section6c2 %>%
  left_join(
    section6c1 %>% select(personid, row_id, v630),
...
s6c3_missing <- s6c3_qualified %>%
  filter(!personid %in% section6c3$personid)
...
s6c3_missing <- s6c3_missing %>%
  mutate(
    v649a = case_when(
      v630 == 1 ~ as.character(sample(c(11, 12), n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649b = case_when(
      v630 == 2 ~ as.character(sample(c(21, 22, 23), n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649c = case_when(
      v630 == 3 ~ "31",
    ...
    v649d = case_when(
      v630 == 4 ~ as.character(sample(c(41, 42, 43, 44), n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649e = case_when(
      v630 == 5 ~ as.character(sample(c(51, 52), n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649f = case_when(
      v630 == 6 ~ as.character(sample(c(61, 62), n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649g = case_when(
      v630 == 7 ~ as.character(sample(71:76, n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649h = case_when(
      v630 == 8 ~ as.character(sample(81:83, n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649i = case_when(
      v630 == 9 ~ as.character(sample(c(91, 92), n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649j = case_when(
      v630 == 10 ~ "101",
    ...
    v649k = case_when(
      v630 == 11 ~ as.character(sample(c(111, 112), n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649l = case_when(
      v630 == 12 ~ as.character(sample(121:124, n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649m = case_when(
      v630 == 13 ~ as.character(sample(131:133, n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v649n = case_when(
      v630 == 14 ~ as.character(sample(c(141, 142), n(), replace = TRUE)),
      TRUE ~ NA_character_
    ), 
    v650 = case_when(
      v630 %in% 1:14 ~ as.character(sample(1:5, n(), replace = TRUE)),
      TRUE ~ NA_character_
    )
  ) %>%
  select(-v629, -v630)

write.xlsx(s6c3_missing, "s6c3_missing.xlsx")
...
section6c4 <- section6c4 %>%
  select(-hhid, -uniq_id, -disease_id)
...
remittance_qualified <- section1a %>%
...
remittance_missing <- merge(
  remittance_missing, 
  section1a[, c("personid", "v109", "v101")],
  by = "personid"
)
...
districts <- toupper(unique(haven::as_factor(section1a$district)))
...
sum(remittance_qualified$personid %in% section12a$personid)
